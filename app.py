...
def page_home():
   ...
   실제 논문 데이터에 근거하여 각 공정별 공정변수에 대한 가동 범위와 불량률 함수, 상관관계를 
    anaconda spyder 프로그램을 통해 설정하였고 가상데이터 10000개를 생성함.
    물론 실제 공정을 제대로 정확하게 오차없이 모델링하여 반영한다면 공정변수를 더 구체적으로 많이 설정해야함. 
    서로 관계가 있고 영향이 있는 공정변수는 셀 수 없이 더 많지만 특정 공정별로 세분화하여 어느정도의 변화를 보이기 위하여
    각 공정별로 환경변수 포함 6개의 공정변수, 와이어 본딩 공정은 10개의 공정변수를 택함.
    생성된 10000개의 데이터에 기반하여 반도체 패키징 공정의 불량률을 예측하고 어느 조합, 어느 특정 값에서 불량률이 높은지
    이상치가 있는지, 불량률이 높은 조건이 무엇인지를 판별할 수 있는 패키징 공정 불량률 예측 프로그램임.
    상용화를 고려한다면 실제 기업에서 실제 장비로 연구하여 직접 공정변수의 범위를 설정하여 재설정 가능하고
    상관관계 또한 공정간의 관계 또한 기업 내에서 연구팀이 연구하여 재설정 가능함 
