...
def page_home():
    ...
    본 시스템은 패키징 공정 단계에서 수집된 다양한 변수(예: 온도, 압력, 시간 등)를 기반으로,
    AI 예측 모델을 통해 **불량률을 예측**하고 이를 시각화함으로써,
    **공정 최적화** 및 **문제 조기 대응**이 가능하도록 도와줍니다.

    아래는 본 시스템의 구성도입니다.
    """)
    
    # 시스템 구조도 이미지 삽입 (업로드된 이미지 사용)
    safe_image("images/Package1.JPG", caption="반도체 패키징 공정 불량률 예측 시스템 구조도", use_container_width=True)
